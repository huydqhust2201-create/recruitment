...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    axiosInstance
      ...
      .then((res) => {
        ...
        setSkillIdCounter(job.skills.length + 1);
      })
      .catch((err: unknown) => {
        toast.error(err instanceof Error ? err.message : 'Không tìm thấy job');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axiosInstance.put(`/api/recruiter/jobs/${id}`, payload);
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Cập nhật thông tin công việc</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-3 pb-4">
        <Button variant="outline" onClick={() => router.push(`/recruiter/jobs/${id}`)} disabled={saving}>
          Hủy
        </Button>
